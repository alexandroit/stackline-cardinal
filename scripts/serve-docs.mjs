...
createServer(async (request, response) => {
  try {
    ...
    if (file !== root && !file.startsWith(`${root}${path.sep}`)) return sendError(response, 403, 'Forbidden')
...
}).listen(port, host, () => console.log(`Documentation available at http://127.0.0.1:${port}`))
